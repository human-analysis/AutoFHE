...
void autofhe_ReLU_seal(long comp_no, vector<int> deg, vector<vector<double>> &decomp_coeff, vector<Tree> &tree, double B, Encryptor &encryptor, Evaluator &evaluator, Decryptor &decryptor, CKKSEncoder &encoder, PublicKey &public_key, SecretKey &secret_key, RelinKeys &relin_keys, Ciphertext& cipher_in, Ciphertext& cipher_res)
{
	// variables
	Plaintext plain_half;
	Ciphertext cipher_temp, cipher_half, cipher_x;
	...
	coeff.reserve(comp_no);
	for (size_t i = 0; i < comp_no; i++)
	{
		vector<double> tmp = decomp_coeff[i];
		coeff.emplace_back(tmp);
	}
	
	// scale coefficients to integrate B
	for(int j=0; j<coeff_number(deg[comp_no-1], tree[comp_no-1]); j++) coeff[comp_no-1][j] *= B;		// scale 


	// generation of half ciphertext
	long n = cipher_in.poly_modulus_degree()/2;
	vector<double> m_half(n, 0.5 * B);
	cipher_x = cipher_in;

	// evaluating (pk ... p1(x)) * B
	for (int i = 0; i < comp_no; ++i) eval_polynomial_integrate(encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, cipher_x, cipher_x, deg[i], coeff[i], tree[i]);  

	// x(0.5B+0.5B*sgn(x)) from sgn(x)*0.5B
	encoder.encode(m_half, cipher_x.scale(), plain_half);
	encryptor.encrypt(plain_half, cipher_half);	
	cout << cipher_half.scale() << endl;
	evaluator.add_reduced_error(cipher_x, cipher_half, cipher_temp);
	evaluator.multiply_reduced_error(cipher_temp, cipher_in, relin_keys, cipher_res);
	cout << cipher_res.scale() << endl;
	evaluator.rescale_to_next_inplace(cipher_res);
	cout << cipher_res.scale() << endl;
}